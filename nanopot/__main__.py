...
# using the logging module to log to a file and to the console at the same time (if the user wants to)
import logging
# we are able to replace the print() statement with the logger.info() statement


# from the nanopot package that we have we import the HoneyPot class that is we import it from the __init__.py file
...
log_filepath = config.get('default', 'logfile', raw=True,
                          fallback="/var/log/nanopot.log")  # we are getting the log_filepath from the config file
print("TEST")
